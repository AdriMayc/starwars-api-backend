...
def test_starships_list_success_adds_id_and_envelope():
    respx.get("https://swapi.dev/api/starships/").respond(
        200,
        json={
            "count": 1,
            "results": [
...
def test_starships_list_timeout_returns_504():
    respx.get("https://swapi.dev/api/starships/").side_effect = httpx.ReadTimeout("boom")
...
def test_starships_list_search_forwards_to_swapi():
    route = respx.get("https://swapi.dev/api/starships/").respond(
        200,
        json={
            "count": 1,
            "results": [
